			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="finished" title="${message(code: 'tournament.finished.label', default: 'Finished')}" />
				...
				<g:each in="${tournamentInstanceList}" status="i" var="tournamentInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${tournamentInstance.id}">${fieldValue(bean: tournamentInstance, field: "dateCreated")}</g:link></td>
					
						<td><g:formatBoolean boolean="${tournamentInstance.finished}" /></td>
